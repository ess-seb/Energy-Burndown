...
- [x] T009 [US1] Add private method `_computeFullEnd(period: ComparisonPeriod): Date` in `src/card/cumulative-comparison-chart.ts`; if `this._config.comparison_mode === 'year_over_year'` return `new Date(period.current_start.getFullYear(), 11, 31)`; else (month_over_year) return `new Date(period.current_start.getFullYear(), period.current_start.getMonth() + 1, 0)` (day 0 of next month = last day of current month)
